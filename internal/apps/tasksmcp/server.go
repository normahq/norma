...
func (s *service) registerTools(server *mcp.Server) {
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_add", Description: "Create a task."}, s.addTask)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_add_epic", Description: "Create an epic."}, s.addEpic)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_add_feature", Description: "Create a feature under an epic."}, s.addFeature)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_add_follow_up", Description: "Create a follow-up task under a parent."}, s.addFollowUp)

	mcp.AddTool(server, &mcp.Tool{Name: "tasks_list", Description: "List tasks, optionally by status."}, s.listTasks)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_list_features", Description: "List features under an epic."}, s.listFeatures)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_children", Description: "List child tasks for a parent task."}, s.listChildren)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_get", Description: "Get a task by id."}, s.getTask)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_leaf", Description: "List ready leaf tasks."}, s.leafTasks)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_list_blocked_dependents", Description: "List tasks blocked by the given task id."}, s.listBlockedDependents)

	mcp.AddTool(server, &mcp.Tool{Name: "tasks_mark_done", Description: "Mark task as done."}, s.markDone)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_mark_status", Description: "Mark task with status."}, s.markStatus)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_update_workflow_state", Description: "Update granular workflow state label."}, s.updateWorkflowState)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_update", Description: "Update task title and goal."}, s.updateTask)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_delete", Description: "Delete a task."}, s.deleteTask)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_set_run", Description: "Set external run id for a task."}, s.setRun)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_set_notes", Description: "Set task notes."}, s.setNotes)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_close_with_reason", Description: "Close a task with explicit reason."}, s.closeWithReason)

	mcp.AddTool(server, &mcp.Tool{Name: "tasks_add_dependency", Description: "Add dependency: task_id depends on depends_on_id."}, s.addDependency)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_add_related_link", Description: "Add bidirectional related link between two tasks."}, s.addRelatedLink)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_add_label", Description: "Add a label to a task."}, s.addLabel)
	mcp.AddTool(server, &mcp.Tool{Name: "tasks_remove_label", Description: "Remove a label from a task."}, s.removeLabel)
}
...
func (s *service) addTask(ctx context.Context, _ *mcp.CallToolRequest, in addTaskInput) (*mcp.CallToolResult, addTaskOutput, error) {
	...
	if title == "" {
		result, out := validationFailure("tasks_add", "title is required")
	...
	if err != nil {
		result, out := backendFailure("tasks_add", err)
		return result, addTaskOutput{ToolOutcome: out}, nil
	}
...
func (s *service) addFeature(ctx context.Context, _ *mcp.CallToolRequest, in addFeatureInput) (*mcp.CallToolResult, addFeatureOutput, error) {
	...
	if epicID == "" {
		result, out := validationFailure("tasks_add_feature", "epic_id is required")
		return result, addFeatureOutput{ToolOutcome: out}, nil
	}
	title := strings.TrimSpace(in.Title)
	if title == "" {
		result, out := validationFailure("tasks_add_feature", "title is required")
		return result, addFeatureOutput{ToolOutcome: out}, nil
	}

	id, err := s.tracker.AddFeature(ctx, epicID, title)
	if err != nil {
		result, out := backendFailure("tasks_add_feature", err)
		return result, addFeatureOutput{ToolOutcome: out}, nil
	}
...
func (s *service) addFollowUp(ctx context.Context, _ *mcp.CallToolRequest, in addFollowUpInput) (*mcp.CallToolResult, addFollowUpOutput, error) {
	...
	if parentID == "" {
		result, out := validationFailure("tasks_add_follow_up", "parent_id is required")
		return result, addFollowUpOutput{ToolOutcome: out}, nil
	}
	title := strings.TrimSpace(in.Title)
	if title == "" {
		result, out := validationFailure("tasks_add_follow_up", "title is required")
		return result, addFollowUpOutput{ToolOutcome: out}, nil
	}

	id, err := s.tracker.AddFollowUp(ctx, parentID, title, strings.TrimSpace(in.Goal), toCriteria(in.Criteria))
	if err != nil {
		result, out := backendFailure("tasks_add_follow_up", err)
		return result, addFollowUpOutput{ToolOutcome: out}, nil
	}
...
func (s *service) listTasks(ctx context.Context, _ *mcp.CallToolRequest, in listTasksInput) (*mcp.CallToolResult, listTasksOutput, error) {
	...
	if err != nil {
		result, out := backendFailure("tasks_list", err)
...
func (s *service) listFeatures(ctx context.Context, _ *mcp.CallToolRequest, in listFeaturesInput) (*mcp.CallToolResult, listFeaturesOutput, error) {
	epicID := strings.TrimSpace(in.EpicID)
	if epicID == "" {
		result, out := validationFailure("tasks_list_features", "epic_id is required")
		return result, listFeaturesOutput{ToolOutcome: out}, nil
	}

	items, err := s.tracker.ListFeatures(ctx, epicID)
	if err != nil {
		result, out := backendFailure("tasks_list_features", err)
		return result, listFeaturesOutput{ToolOutcome: out}, nil
	}
...
func (s *service) listChildren(ctx context.Context, _ *mcp.CallToolRequest, in childrenInput) (*mcp.CallToolResult, childrenOutput, error) {
	parentID := strings.TrimSpace(in.ParentID)
	if parentID == "" {
		result, out := validationFailure("tasks_children", "parent_id is required")
		return result, childrenOutput{ToolOutcome: out}, nil
	}

	items, err := s.tracker.Children(ctx, parentID)
	if err != nil {
		result, out := backendFailure("tasks_children", err)
		return result, childrenOutput{ToolOutcome: out}, nil
	}
...
func (s *service) getTask(ctx context.Context, _ *mcp.CallToolRequest, in getTaskInput) (*mcp.CallToolResult, getTaskOutput, error) {
	...
	if id == "" {
		result, out := validationFailure("tasks_get", "id is required")
		return result, getTaskOutput{ToolOutcome: out}, nil
	}

	item, err := s.tracker.Task(ctx, id)
	if err != nil {
		result, out := backendFailure("tasks_get", err)
		return result, getTaskOutput{ToolOutcome: out}, nil
	}
...
func (s *service) markDone(ctx context.Context, _ *mcp.CallToolRequest, in idInput) (*mcp.CallToolResult, basicOutput, error) {
	id := strings.TrimSpace(in.ID)
	if id == "" {
		result, out := validationFailure("tasks_mark_done", "id is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}

	if err := s.tracker.MarkDone(ctx, id); err != nil {
		result, out := backendFailure("tasks_mark_done", err)
		return result, basicOutput{ToolOutcome: out}, nil
	}
...
func (s *service) markStatus(ctx context.Context, _ *mcp.CallToolRequest, in markStatusInput) (*mcp.CallToolResult, basicOutput, error) {
	id := strings.TrimSpace(in.ID)
	if id == "" {
		result, out := validationFailure("tasks_mark_status", "id is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}
	status := strings.TrimSpace(in.Status)
	if status == "" {
		result, out := validationFailure("tasks_mark_status", "status is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}

	if err := s.tracker.MarkStatus(ctx, id, status); err != nil {
		result, out := backendFailure("tasks_mark_status", err)
		return result, basicOutput{ToolOutcome: out}, nil
	}
	return nil, basicOutput{ToolOutcome: okOutcome()}, nil
}
...
func (s *service) updateTask(ctx context.Context, _ *mcp.CallToolRequest, in updateTaskInput) (*mcp.CallToolResult, basicOutput, error) {
	id := strings.TrimSpace(in.ID)
	if id == "" {
		result, out := validationFailure("tasks_update", "id is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}
	title := strings.TrimSpace(in.Title)
	if title == "" {
		result, out := validationFailure("tasks_update", "title is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}

	if err := s.tracker.Update(ctx, id, title, strings.TrimSpace(in.Goal)); err != nil {
		result, out := backendFailure("tasks_update", err)
		return result, basicOutput{ToolOutcome: out}, nil
	}
	return nil, basicOutput{ToolOutcome: okOutcome()}, nil
}
...
func (s *service) deleteTask(ctx context.Context, _ *mcp.CallToolRequest, in idInput) (*mcp.CallToolResult, basicOutput, error) {
	id := strings.TrimSpace(in.ID)
	if id == "" {
		result, out := validationFailure("tasks_delete", "id is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}

	if err := s.tracker.Delete(ctx, id); err != nil {
		result, out := backendFailure("tasks_delete", err)
		return result, basicOutput{ToolOutcome: out}, nil
	}
	return nil, basicOutput{ToolOutcome: okOutcome()}, nil
}
...
func (s *service) setRun(ctx context.Context, _ *mcp.CallToolRequest, in setRunInput) (*mcp.CallToolResult, basicOutput, error) {
	id := strings.TrimSpace(in.ID)
	if id == "" {
		result, out := validationFailure("tasks_set_run", "id is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}
	runID := strings.TrimSpace(in.RunID)
	if runID == "" {
		result, out := validationFailure("tasks_set_run", "run_id is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}

	if err := s.tracker.SetRun(ctx, id, runID); err != nil {
		result, out := backendFailure("tasks_set_run", err)
		return result, basicOutput{ToolOutcome: out}, nil
	}
	return nil, basicOutput{ToolOutcome: okOutcome()}, nil
}
...
func (s *service) addDependency(ctx context.Context, _ *mcp.CallToolRequest, in addDependencyInput) (*mcp.CallToolResult, basicOutput, error) {
	...
	if taskID == "" {
		result, out := validationFailure("tasks_add_dependency", "task_id is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}
	dependsOnID := strings.TrimSpace(in.DependsOnID)
	if dependsOnID == "" {
		result, out := validationFailure("tasks_add_dependency", "depends_on_id is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}

	if err := s.tracker.AddDependency(ctx, taskID, dependsOnID); err != nil {
		result, out := backendFailure("tasks_add_dependency", err)
		return result, basicOutput{ToolOutcome: out}, nil
	}
	return nil, basicOutput{ToolOutcome: okOutcome()}, nil
}
...
func (s *service) leafTasks(ctx context.Context, _ *mcp.CallToolRequest, _ noInput) (*mcp.CallToolResult, listTasksOutput, error) {
	...
	if err != nil {
		result, out := backendFailure("tasks_leaf", err)
		return result, listTasksOutput{ToolOutcome: out}, nil
	}
	return nil, listTasksOutput{ToolOutcome: okOutcome(), Tasks: mapTasks(items)}, nil
}
...
func (s *service) updateWorkflowState(ctx context.Context, _ *mcp.CallToolRequest, in workflowStateInput) (*mcp.CallToolResult, basicOutput, error) {
	id := strings.TrimSpace(in.ID)
	if id == "" {
		result, out := validationFailure("tasks_update_workflow_state", "id is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}
	state := strings.TrimSpace(in.State)
	if state == "" {
		result, out := validationFailure("tasks_update_workflow_state", "state is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}

	if err := s.tracker.UpdateWorkflowState(ctx, id, state); err != nil {
		result, out := backendFailure("tasks_update_workflow_state", err)
		return result, basicOutput{ToolOutcome: out}, nil
	}
	return nil, basicOutput{ToolOutcome: okOutcome()}, nil
}
...
func (s *service) addLabel(ctx context.Context, _ *mcp.CallToolRequest, in labelInput) (*mcp.CallToolResult, basicOutput, error) {
	id := strings.TrimSpace(in.ID)
	if id == "" {
		result, out := validationFailure("tasks_add_label", "id is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}
	label := strings.TrimSpace(in.Label)
	if label == "" {
		result, out := validationFailure("tasks_add_label", "label is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}

	if err := s.tracker.AddLabel(ctx, id, label); err != nil {
		result, out := backendFailure("tasks_add_label", err)
		return result, basicOutput{ToolOutcome: out}, nil
	}
	return nil, basicOutput{ToolOutcome: okOutcome()}, nil
}
...
func (s *service) removeLabel(ctx context.Context, _ *mcp.CallToolRequest, in labelInput) (*mcp.CallToolResult, basicOutput, error) {
	id := strings.TrimSpace(in.ID)
	if id == "" {
		result, out := validationFailure("tasks_remove_label", "id is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}
	label := strings.TrimSpace(in.Label)
	if label == "" {
		result, out := validationFailure("tasks_remove_label", "label is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}

	if err := s.tracker.RemoveLabel(ctx, id, label); err != nil {
		result, out := backendFailure("tasks_remove_label", err)
		return result, basicOutput{ToolOutcome: out}, nil
	}
	return nil, basicOutput{ToolOutcome: okOutcome()}, nil
}
...
func (s *service) setNotes(ctx context.Context, _ *mcp.CallToolRequest, in setNotesInput) (*mcp.CallToolResult, basicOutput, error) {
	id := strings.TrimSpace(in.ID)
	if id == "" {
		result, out := validationFailure("tasks_set_notes", "id is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}

	if err := s.tracker.SetNotes(ctx, id, in.Notes); err != nil {
		result, out := backendFailure("tasks_set_notes", err)
		return result, basicOutput{ToolOutcome: out}, nil
	}
	return nil, basicOutput{ToolOutcome: okOutcome()}, nil
}
...
func (s *service) closeWithReason(ctx context.Context, _ *mcp.CallToolRequest, in closeWithReasonInput) (*mcp.CallToolResult, basicOutput, error) {
	id := strings.TrimSpace(in.ID)
	if id == "" {
		result, out := validationFailure("tasks_close_with_reason", "id is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}
	reason := strings.TrimSpace(in.Reason)
	if reason == "" {
		result, out := validationFailure("tasks_close_with_reason", "reason is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}

	if err := s.tracker.CloseWithReason(ctx, id, reason); err != nil {
		result, out := backendFailure("tasks_close_with_reason", err)
		return result, basicOutput{ToolOutcome: out}, nil
	}
	return nil, basicOutput{ToolOutcome: okOutcome()}, nil
}
...
func (s *service) addRelatedLink(ctx context.Context, _ *mcp.CallToolRequest, in addRelatedLinkInput) (*mcp.CallToolResult, basicOutput, error) {
	...
	if from == "" {
		result, out := validationFailure("tasks_add_related_link", "from_id is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}
	to := strings.TrimSpace(in.ToID)
	if to == "" {
		result, out := validationFailure("tasks_add_related_link", "to_id is required")
		return result, basicOutput{ToolOutcome: out}, nil
	}

	if err := s.tracker.AddRelatedLink(ctx, from, to); err != nil {
		result, out := backendFailure("tasks_add_related_link", err)
		return result, basicOutput{ToolOutcome: out}, nil
	}
	return nil, basicOutput{ToolOutcome: okOutcome()}, nil
}
...
func (s *service) listBlockedDependents(ctx context.Context, _ *mcp.CallToolRequest, in idInput) (*mcp.CallToolResult, listTasksOutput, error) {
	id := strings.TrimSpace(in.ID)
	if id == "" {
		result, out := validationFailure("tasks_list_blocked_dependents", "id is required")
		return result, listTasksOutput{ToolOutcome: out}, nil
	}

	items, err := s.tracker.ListBlockedDependents(ctx, id)
	if err != nil {
		result, out := backendFailure("tasks_list_blocked_dependents", err)
		return result, listTasksOutput{ToolOutcome: out}, nil
	}
	return nil, listTasksOutput{ToolOutcome: okOutcome(), Tasks: mapTasks(items)}, nil
}
